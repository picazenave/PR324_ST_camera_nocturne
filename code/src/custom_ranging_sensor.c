...
static int32_t VL53L5CX_Probe(uint32_t Instance)
{
  ...
  {
    printf("error VL53L5CX_RegisterBusIO\r\n");
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }
  else
  {
    ...
    ret=VL53L5CX_ReadID(&VL53L5CXObj, &id);
    if (ret != VL53L5CX_OK)
    {
      printf("error VL53L5CX_ReadID=%ld\r\n",ret);
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
    ...
    {
      printf("error CUSTOM_RANGING_Drv[Instance]->Init\r\n");
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
    ...
    {
      printf("error CUSTOM_RANGING_Drv[Instance]->GetCapabilities\r\n");
      ret = BSP_ERROR_COMPONENT_FAILURE;
    }
    else
    {
